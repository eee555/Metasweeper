...
from PySide6.QtWidgets import QTableWidgetItem, QAbstractItemDelegate, QApplication
...
class ui_Form(Ui_Form):
    ...
    def __init__(self, pix_size, parent):
        ...
        self.QWidget.setWindowIcon(QtGui.QIcon(
            str(resource_path('media') / 'cat.ico')))
    ...
    def show(self, index_value_list: list[str]):
        ...
        for idx in range(self.tableWidget.rowCount()):
            self.tableWidget.setItem(
                idx, 1, QTableWidgetItem(index_value_list[idx]))
...
class gameScoreBoardManager():
    ...
    def __init__(self, score_board_setting, game_setting, pix_size, parent):
        ...
        # self.ms_board = None
        self.namespace = {}
        self.scoreboard_pixsize = game_setting.value(
            "DEFAULT/scoreboard_pixsize", pix_size, int)
        self.scoreboard_pixsize = max(MIN_PIX_SIZE, min(
            MAX_PIX_SIZE, self.scoreboard_pixsize))
        ...
        self.score_board_items = self.score_board_setting.get_or_set_section(
    ...
    def reshow(self, ms_board, index_type=0):
        ...
        index_value_list = self.cal_index_value(ms_board, index_type)
        self.ui.reshow([i[0]
                       for i in self.score_board_items], index_value_list)
        # self.visible()
        ...
        ...
        if e is None or (self.editing_row >= 0 and self.editing_column >= 0 and (self.editing_row != e.row() or
            ...
            if self.editing_column == 0:
                if not new_formula:
                    ...
                    # 正常修改键名
                    self.score_board_items[self.editing_row] = (
                        new_formula, self.score_board_items[self.editing_row][1])
            else:
                # 正常修改公式
                self.score_board_items[self.editing_row] = (
    ...
    def close(self):
        ...
        self.game_setting.set_value(
            "DEFAULT/scoreboardtop", self.ui.QWidget.x())
        self.game_setting.set_value(
            "DEFAULT/scoreboardleft", self.ui.QWidget.y())
        self.game_setting.set_value(
            "DEFAULT/scoreboard_pixsize", self.scoreboard_pixsize)
