...
from PySide6.QtCore import Qt, QRectF, Signal, QStringListModel, QSortFilterProxyModel
from PySide6.QtGui import QPainter, QPainterPath, QBrush, QColor, QPixmap
...
class RoundQWidget(RoundMixin, QWidget):
    barSetMineNum = QtCore.Signal(int)
    barSetMineNumCalPoss = QtCore.Signal()
    closeEvent_ = Signal()
...
class RoundQDialog(RoundMixin, QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._init_round()
        self.setStyleSheet(GLOBAL_QSS)
...
class StatusLabel (QtWidgets.QLabel):
    # 最上面的脸的控件，在这里重写一些方法
    leftRelease = QtCore.Signal()  # 定义信号
    ...
    def reloadFace(self, pixSize):
        ...
        self.pixmap1 = QPixmap(self.smilefacedown_path).scaled(
            int(pixSize * 1.5), int(pixSize * 1.5))
        self.pixmap2 = QPixmap(self.smileface_path).scaled(
            int(pixSize * 1.5), int(pixSize * 1.5))
    ...
    def setPath(self):
        self.smileface_path = str(resource_path(
            'media').joinpath('smileface.svg'))
        self.smilefacedown_path = str(resource_path(
            'media').joinpath('smilefacedown.svg'))
