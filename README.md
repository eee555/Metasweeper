...
* **历史记录插件**：自动记录每局游戏的完整数据，支持右键菜单以 JSON 格式复制任一条记录，提供多种格式的导入/导出（stats.dat / meta.dat / textstats.csv），支持"计算列"功能。
* **LLM 反控扫雷插件**：通过大语言模型以自然语言指令远程控制游戏（如"打开中级"、"标雷"），实现 AI 与游戏的直接对话。
* **雷修境界插件**：经验 / 升级 / 抽签系统，将扫雷练习游戏化，集成视频吸收自动判断机制。
* **统计插件 v1.0.0**：提供游戏数据统计与分析功能。
* **无猜关卡插件 v1.0.0-beta**：提供无猜模式关卡挑战。
...
- 新增无猜关卡插件v1.0.0-beta
- 历史记录插件现在支持“计算列”
- 现在计数器和扫雷界面窗口大小可以分别按下“ctrl”并滚动滚轮独立调整
