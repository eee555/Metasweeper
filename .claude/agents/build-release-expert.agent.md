...
- **Pylance 工具优先**：凡是 Pylance 工具能完成的任务（如查看/分析代码文件、检查依赖模块是否已安装、查询环境信息等），一律优先使用 Pylance 工具，不要使用终端命令
- 仅当 Pylance 工具无法胜任时（如 `pyinstaller` 打包、`iscc` 编译安装程序、运行构建脚本等本身就是核心职责的命令），才使用终端命令
